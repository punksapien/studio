...
import { 
  ...
  ChevronLeft, ChevronRight // Added for image navigation
} from 'lucide-react';
...
function ImageGallery({ imageUrls, listingTitle }: { imageUrls?: string[]; listingTitle: string }) {
  ...
  React.useEffect(() => {
    // Reset index if imageUrls change and currentIndex is out of bounds
    if (currentIndex >= validImageUrls.length) {
      setCurrentIndex(0);
    }
  }, [validImageUrls, currentIndex]);
  
  const mainImage = validImageUrls.length > 0 ? validImageUrls[currentIndex] : "https://placehold.co/1200x675.png?text=No+Image";
  ...
  if (validImageUrls.length === 0) {
    return (
      <div className="mb-4 rounded-lg overflow-hidden shadow-lg bg-muted aspect-[16/9] flex items-center justify-center">
        <Image 
          src="https://placehold.co/1200x400.png?text=No+Image+Available" 
  ...
  return (
    <div>
      <div className="mb-4 rounded-lg overflow-hidden shadow-lg bg-muted aspect-[16/9] flex items-center justify-center relative">
        <Image
          src={mainImage}
          ...
          priority
          key={mainImage} // Add key to force re-render on src change if needed
        ...
        {validImageUrls.length > 1 && (
          <>
            <Button
              variant="outline"
              size="icon"
              className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white border-none rounded-full"
            ...
            <Button
              variant="outline"
              size="icon"
              className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-white border-none rounded-full"
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
          {validImageUrls.map((url, index) => (
            <button
              ...
              className={cn(
                "aspect-square rounded-md overflow-hidden focus:outline-none transition-opacity",
            ...
            >
              <Image 
                ...
                onError={(e) => (e.currentTarget.src = "https://placehold.co/150x150.png?text=Error")}
              />
            </button>
          ))}
        </div>
...
export default function ListingDetailPage() {
  ...
  // User 'user2' is Jane Smith (Verified & Paid Buyer)
  // User 'user5' is Michael Lee (Verified but Free Buyer)
  // User 'user6' is Anna Tay (Anonymous Buyer)
