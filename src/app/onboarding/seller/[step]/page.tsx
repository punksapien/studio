...
import { updateOnboardingStatus, uploadOnboardingDocument, updateUserProfile } from '@/hooks/use-current-user'; // Added updateUserProfile
...
const Step1SellerSchema = z.object({
  ...
  countryOfOperation: z.string().min(2, "Country is required."), // Assuming this will be a select later
...
const Step3SellerSchema = z.object({
  ...
  proofOfOwnershipFile: z.instanceof(File, { message: "Proof of ownership document is required." })
    .refine(file => file.size <= 5 * 1024 * 1024, "File size must be 5MB or less.")
    .refine(file => ['image/jpeg', 'image/png', 'application/pdf'].includes(file.type), "Invalid file type. JPG, PNG, PDF only.")
    .optional(), // Making this optional for now as per schema
});
...
type FormValues = Partial<z.infer<typeof Step1SellerSchema>> &
                  ...
                  { submitted_documents?: Record<string, boolean> }; // For tracking successful uploads
...
export default function SellerOnboardingStepPage() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      if (currentStep === 1) {
        await updateUserProfile({ // Assuming this function exists in useCurrentUser or similar
          initial_company_name: updatedData.registeredBusinessName!,
          country: updatedData.countryOfOperation!,
          // Add other profile fields if they are directly on user_profiles
        });
        // We can store business overview details in `submitted_documents` or a dedicated field in `user_profiles`
        documentsToSubmitThisStep = {
          ...documentsToSubmitThisStep,
          business_overview: true // Indicates business overview step data is captured
        };
      ...
      // After successful operations for the current step (profile update or document upload)
      await updateOnboardingStatus({
        step_completed: currentStep,
        submitted_documents: documentsToSubmitThisStep, // Pass the accumulated/updated doc status
        complete_onboarding: currentStep === totalSteps // Mark complete only on final step
      });

      updatedData.submitted_documents = documentsToSubmitThisStep; // Ensure formData reflects this too
