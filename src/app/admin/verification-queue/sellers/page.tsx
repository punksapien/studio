...
import { Eye, FileText, Edit, ShieldCheck, AlertTriangle, MailOpen, MessageSquare, Clock, FileSearch, Briefcase, RefreshCw, Loader2, Users, InboxIcon, Filter, XCircle } from "lucide-react";
...
function AdminSellerVerificationQueuePageContent() {
  ...
  const apiUrl = `/api/admin/verification-queue/sellers?page=1&limit=500&status=${statusFilter}`;
  ...
  if (error) {
    return (
      <AdminPageShell title="Seller Verification" description="Review and manage sellers and their listings awaiting verification.">
  ...
  return (
    <AdminPageShell
      ...
      description="Review and manage sellers and their listings awaiting verification."
      actions={
        <>
...
export default function AdminSellerVerificationQueuePage() {
  ...
  return (
    <React.Suspense fallback={null}>
      <AdminSellerVerificationQueuePageContent />
    </React.Suspense>
  );
}
