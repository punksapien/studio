...
function AdminBuyerVerificationQueuePageContent() {
  ...
  const apiUrl = `/api/admin/verification-queue/buyers?page=1&limit=500&status=${statusFilter}`;
  ...
  if (error) {
    return (
      <AdminPageShell title="Buyer Verification" description="Review and manage buyers awaiting verification.">
  ...
  return (
    <AdminPageShell
      ...
      description="Review and manage buyers awaiting verification."
      actions={
        <>
          <Select value={statusFilter} onValueChange={handleStatusFilterChange}>
            <SelectTrigger className="w-48 h-9 text-xs bg-background border-input"><SelectValue placeholder="Filter by status" /></SelectTrigger>
            <SelectContent>{["all", "New Request", "Contacted", "Docs Under Review", "More Info Requested", "Approved", "Rejected"].map(s => <SelectItem key={s} value={s}>{s === 'all' ? 'All' : s}</SelectItem>)}</SelectContent>
          </Select>
    ...
    >
            ...
            <Table className="w-full">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="whitespace-nowrap w-44 text-muted-foreground">Buyer Name</TableHead>
                  <TableHead className="w-52 text-muted-foreground">Email</TableHead>
                  ...
                  <TableHead className="text-right w-32 text-muted-foreground">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-10">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <p className="text-sm font-medium text-foreground">The buyer verification queue is empty.</p>
                  ...
                  requests.map((req) => (
                    <TableRow key={req.id} className="text-sm hover:bg-muted/50">
                      ...
                      <TableCell className="text-right whitespace-nowrap">
                        ...
                        <Button variant="ghost" size="icon" asChild title="View Buyer Details" className="text-muted-foreground hover:text-accent ml-1">
          ...
          <p className="text-sm text-muted-foreground">
            Total pending: {pendingCount} | Total requests: {data?.pagination.total || 0}
          </p>
