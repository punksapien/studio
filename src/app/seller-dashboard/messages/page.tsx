...
export default function SellerMessagesPage() {
  ...
  if (isLoading) {
    return (
      <DashboardPageShell
        title="Messages"
        description="Your ongoing conversations with potential buyers."
        scrollable
      >
            ...
            <div className="space-y-4">
              {conversations.map((conv) => (
                <Link key={conv.conversationId} href={`/seller-dashboard/messages/${conv.conversationId}`} passHref>
                  <div className="block p-4 border border-brand-light-gray transition-colors cursor-pointer hover:border-brand-sky-blue">
