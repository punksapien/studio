                ...
                <div className="flex justify-between items-center">
                  <CardTitle>Business Listings by {user.fullName}</CardTitle>
                   <Button variant="outline" size="sm" asChild>
                      <Link href={`/app/admin/listings?sellerId=${user.id}`}>View All Listings ({userListings.length})</Link>
                  </Button>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  <ul className="space-y-4">
                    {userListings.slice(0,3).map(listing => ( 
                      <li key={listing.id} className="p-4 border rounded-lg flex justify-between items-center hover:bg-muted/50 transition-colors">
                        <div>
                          <Link href={`/app/admin/listings/${listing.id}`} className="font-medium text-primary hover:underline">{listing.listingTitleAnonymous}</Link>
                          <p className="text-xs text-muted-foreground">{listing.industry} | Status: {listing.status}</p>
                        </div>
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/app/admin/listings/${listing.id}`}>View Listing</Link>
                        </Button>
                      </li>
                    ))}
                  </ul>
