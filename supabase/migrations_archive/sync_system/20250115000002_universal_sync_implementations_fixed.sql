-- Universal Sync Trigger System - Phase 2: Specific Implementations (FIXED)
-- Migration: 20250115000002_universal_sync_implementations_fixed.sql
--
...
--
-- FIXED: The auth.users trigger now properly handles schema visibility issues
...
-- =============================================================================
-- 8. FIXED AUTH TRIGGER - Properly handles schema visibility
-- =============================================================================

-- IMPORTANT: This is the FIXED version of the auth trigger that caused issues
-- It now properly handles schema visibility and includes error handling

-- First, check if the trigger already exists and drop it
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Create the FIXED trigger function with proper schema qualification
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  -- Use fully qualified table name and handle potential errors
  BEGIN
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      RAISE LOG 'Failed to create/update user profile for user %: %', NEW.id, SQLERRM;
  END;
...
-- Create the trigger with proper schema context
...
-- Recreate triggers with explicit schema references
CREATE TRIGGER trigger_listings_count_sync
  AFTER INSERT OR UPDATE OR DELETE ON public.listings
  FOR EACH ROW
  EXECUTE FUNCTION public.universal_sync_trigger('count_sync');
...
CREATE TRIGGER trigger_user_profiles_status_cascade
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.universal_sync_trigger('status_cascade');

CREATE TRIGGER trigger_user_profiles_audit_trail
  AFTER INSERT OR UPDATE OR DELETE ON public.user_profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.universal_sync_trigger('audit_trail');
...
BEGIN
  RAISE NOTICE '===========================================';
  RAISE NOTICE 'FIXED MIGRATION APPLIED SUCCESSFULLY!';
  RAISE NOTICE 'The auth trigger now properly handles schema visibility';
  RAISE NOTICE 'and includes error handling to prevent auth failures';
  RAISE NOTICE '===========================================';
END $$;
