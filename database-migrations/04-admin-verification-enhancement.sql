...
-- 2. Add enhanced columns to verification_requests table
ALTER TABLE verification_requests
ADD COLUMN IF NOT EXISTS user_notes TEXT, -- Store user's original request notes
ADD COLUMN IF NOT EXISTS admin_notes JSONB DEFAULT '[]', -- Array of admin notes with timestamps
...
ADD COLUMN IF NOT EXISTS days_since_submission INTEGER GENERATED ALWAYS AS (
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.admin_notes IS DISTINCT FROM NEW.admin_notes THEN
    INSERT INTO verification_request_activities (
      ...
      'note_added',
      NULL,
      jsonb_array_length(NEW.admin_notes)::TEXT,
      'Admin note added'
    );
  END IF;
...
COMMENT ON COLUMN verification_requests.admin_notes IS 'JSONB array of admin notes with timestamps';
